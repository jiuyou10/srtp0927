...
const calculateResult = async (
  ...
  if (questionnaire && questionnaire.name) {
    switch (questionnaire.key) {
	  ...
	  case "CPSS": {
        const result = await calculateCPSSResult(answer, db, userId);
        return JSON.stringify(result);
      }
	  case "CPO": {
	  ...
	  case "PTED": {
        const result = await calculatePTEDResult(answer, db, userId);
        return JSON.stringify(result);
      }
	  case "NTT": {
        const result = await calculateNTTResult(answer, db, userId);
        return JSON.stringify(result);
      }
    }
  }
