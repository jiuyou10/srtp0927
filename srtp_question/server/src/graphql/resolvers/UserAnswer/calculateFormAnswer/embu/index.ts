...
export const calculateEmbuResult = async (
  answer: Answer,
  db: Database,
  userId: ObjectId
): Promise<EmbuResult> => {
  ...
  if (isFatherAnswerCompleted) {
    for (let i = 0; i < fatherComponents.length; i++) {
      for (
        ...
        currentQuestionIndex < fatherComponentsIndexes[i].length;
        currentQuestionIndex += 1
      ) {
        const actualChoice =
          answer.answers[
            (Math.abs(fatherComponentsIndexes[i][currentQuestionIndex]) - 1) * 2
          ]; // 0 - 3.
  ...
  if (isMotherAnswerCompleted) {
    for (let i = 0; i < motherComponents.length; i++) {
      for (
        let currentQuestionIndex = 0;
        currentQuestionIndex < motherComponentsIndexes[i].length;
        currentQuestionIndex += 1
      ) {
        const actualChoice =
          answer.answers[
            (Math.abs(motherComponentsIndexes[i][currentQuestionIndex]) - 1) *
              2 +
              1
          ];
