...
exports.generateScl90Report = void 0;
const generateScl90Report = (result, dateString) => {
    if (typeof result !== "string") {
        throw new Error("Type of result should be string!");
    }
    const getPromptText = (itemValue) => {
    ...
    const scl90Result = JSON.parse(result);
    return `
...
>
  <tbody>
    <tr></tr>
    <tr>
      <td width="30%" style="padding-bottom:  8px;">总分：${scl90Result.total}</td>
      <td width="20%"></td>
      <td width="30%">总均分：${scl90Result.generalSymptomaticIndex}</td>
      <td width="20%"></td>
    </tr>
    <tr>
      <td width="30%" style="padding-bottom:  8px;">阳性项目数：${scl90Result.numberOfItemsGreaterThanOrEqualToTwo}</td>
      <td width="20%"></td>
      <td width="30%">阴性项目数：${scl90Result.numberOfItemsLessThanTwo}</td>
      <td width="20%"></td>
    </tr>
    <tr>
      <td width="30%" style="padding-bottom:  8px;">阳性症状痛苦水平：${scl90Result.positiveSymptomDistressLevel}</td>
      <td width="20%"></td>
      <td width="30%">阳性症状均分：${scl90Result.positiveSymptomAverage}</td>
      <td width="20%"></td>
    </tr>
    <tr>
      <td width="30%" style="padding-bottom:  8px;">F1（躯体化）：${scl90Result.f1}</td>
    ...
    <tr>
      <td width="30%" style="padding-bottom:  8px;">F3（人际关系敏感）：${scl90Result.f3}</td>
    ...
    <tr>
      <td width="30%" style="padding-bottom:  8px;">F5（焦虑）：${scl90Result.f5}</td>
    ...
    <tr>
      <td width="30%" style="padding-bottom:  8px;">F7（恐怖）：${scl90Result.f7}</td>
...
exports.generateScl90Report = generateScl90Report;
